...
export class ProductsService {
  ...
  /** Flag que indica si el catálogo fue cargado con el endpoint GEO (incluye nearestStoreKm). */
   ...
   */
  loadCatalog(page = 1, limit = 20): void {
  ...
  /**
   * Carga el catálogo GEO de productos activos desde GET /product/active/geo.
   *
   * Cada producto en la respuesta incluye `nearestStoreKm`:
   *  - Número en km → distancia a la tienda activa con stock más cercana al cliente
   *  - null         → el producto no tiene stock en ninguna tienda activa
   *
   * Prioridad de resolución del cliente (manejada por el backend):
   *  1. lat/lng explícitos → pasados como query params
   *  2. Fallback backend   → Huancayo (-12.0651, -75.2049)
   *
   * Si el catálogo GEO ya fue cargado (caché), no realiza HTTP adicional.
   * Usa forceRefresh=true para invalidar el caché (ej. cuando cambia la ubicación).
   *
   * @param lat Latitud del cliente (obtenida del GeoService)
   * @param lng Longitud del cliente (obtenida del GeoService)
   * @param page Número de página (por defecto 1)
   * @param limit Cantidad de resultados (por defecto 200)
   * @param forceRefresh Si true, ignora el caché y repite el HTTP
   */
  loadGeoCatalog(lat?: number, lng?: number, page = 1, limit = 200, forceRefresh = false): void {
    if (this.geoLoaded && !forceRefresh) return;

    let url = `${this.apiUrl}/active/geo?page=${page}&limit=${limit}`;
    ...
    this.http
      ...
      .pipe(
        catchError((err) => {
          console.error('[ProductsService] Error cargando catálogo GEO:', err);
          return of({ data: [], total: 0, page: 1, limit });
        })
      )
      .subscribe((res) => {
        this._catalog$.next(res.data);
        this.catalogLoaded = true; // También satisface el flag estándar
        this.geoLoaded = true;
      });
  }
   ...
   */
  getMyCatalog(page = 1, limit = 20): Observable<PaginatedResponse> {
