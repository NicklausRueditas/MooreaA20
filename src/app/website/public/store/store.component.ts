...
export class StoreComponent implements OnInit, OnDestroy {
  ...
  private setupDebounces(): void {
    // Fallback de búsqueda al backend via /product/search?tags=
    this.searchTerm$
      .pipe(debounceTime(350), distinctUntilChanged(), takeUntil(this.destroy$))
      .subscribe(query => {
        if (query.trim().length >= 2 && this.products.length === 0) {
          this.fetchByTags(query.trim());
        }
      });

    // Debounce del slider de precio → server-side cuando el valor es < MAX_PRICE
    this.priceSlider$
      .pipe(debounceTime(300), distinctUntilChanged(), takeUntil(this.destroy$))
      .subscribe(price => {
        if (price < MAX_PRICE) {
          // Consulta al backend por rango de precio
          this.fetchByPriceRange(0, price);
        } else {
          // Slider en máximo → restaurar caché del catálogo completo
  ...
  /**
   * Carga el catálogo inicial desde el caché compartido del servicio.
  ...
  applyLocalFilters(): void {
    ...
    // Búsqueda por texto (local, instantánea)
    if (this.searchQuery.trim()) {
      ...
      // Si no hay resultados locales → emitir al stream de fallback
      if (result.length === 0) {
    ...
    // Precio (filtro local rápido sobre lo que ya tenemos en caché)
  ...
  /**
   * Selecciona una categoría. Si es 'all' recarga el catálogo inicial;
  ...
  selectCategory(categoryName: string): void {
    ...
    if (categoryName === 'all') {
      ...
      this.fetchByCategory(categoryName);
    }
  }
  ...
  /** Restablece todos los filtros y recarga el catálogo inicial */
  clearFilters(): void {
    this.searchQuery      = '';
    this.selectedCategory = 'all';
